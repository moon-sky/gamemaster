...
/**
 * Root 后端：执行 `su 0 <cmd>` 实现 shell 截屏与手势注入。
 * 直接迁移自原 [com.gamemaster.agent.screenshot.RootUtil] 的实现，
 * 改造为 [DeviceBackend] 接口实现以便与 Shizuku 后端平级。
 */
object RootBackend : DeviceBackend {
    ...
    override fun exec(cmd: String): Boolean = try {
    ...
    override fun execOut(cmd: String): String? = try {
        val p = ProcessBuilder("su", "0", "sh", "-c", cmd)
    ...
    override fun screenshot(): Bitmap? = try {
    ...
    private fun execInput(vararg args: String): Boolean = try {
