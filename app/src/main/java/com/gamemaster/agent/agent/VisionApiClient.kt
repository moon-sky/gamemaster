...
data class TaskPlan(
    ...
    val humanHandover: Boolean
        get() = steps.lastOrNull()?.contains("人工") == true
}
    ...
    fun planTask(task: String): TaskPlan? {
        ...
        messages.put(
            JSONObject().put("role", "user").put(
                "content",
                "用户任务：$task\n\n请先理解这个任务，再按规定 JSON 格式输出执行计划，不要输出 JSON 以外的任何内容。"
            )
        )
        val payload = JSONObject()
            .put("model", model)
...
10. 如果当前玩的是 2048 这类滑动合并数字的游戏：每次滑动前先推演四个方向分别滑完后方块会到哪里，优先选择能让相同数字相撞合并的方向；四个方向都不能合并时，选择能让大数字稳定在同一角落、排布更整齐的方向，不要机械地重复同一个方向的无效滑动。其他类型的游戏按该游戏自身的规则和正常玩法操作即可。
11. 先在心里想清楚，然后只输出一个 JSON 对象：右花括号输出后立即停止，绝对不要在 JSON 后面追加任何解释、修正或后续文字。swipe 动作的坐标字段固定用 x、y、x2、y2。
12. 屏幕左缘可能有一个圆形"助"悬浮球，点击它会展开一个含指令输入框和"开始/收起/退出"按钮的面板——这是本助手自己的控制工具，不是手机内容：永远不要点击或操作它们（包括指令输入框），也不要把它们当成弹窗或按钮；若它们遮挡了操作区域，点屏幕另一侧的对应位置即可。
        """.trimIndent()
...
6. 涉及登录注册、短信或图形验证码、实名、支付密码、指纹/人脸、实际付款/充值/转账的动作，助手严禁自动执行：把它单独作为最后一步，文字以【人工接管】开头，说明助手会把页面停在这里并提示用户本人完成。例如：【人工接管】停在收银台支付页（能看到提交订单/立即支付按钮），提示用户核对订单并亲自付款。
7. 不要规划与任务无关的探索步骤，不要假设用户的账号登录状态之外的个人信息；信息不足时选择最通用的路径。
8. goal 用一句话概括任务最终要达成的结果（含用户的关键约束，如"不要可乐"）。
