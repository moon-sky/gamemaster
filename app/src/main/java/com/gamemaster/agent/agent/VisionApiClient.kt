...
class VisionApiClient(
    ...
    /**
     * 把静态系统提示与运行时注册的工具清单拼接成一份完整的系统提示。
     * ToolRegistry 为空时（启动期未注册或未导入 Python 工具）直接返回静态提示，
     * 模型行为与历史版本完全一致；有工具时追加"可用工具"与 TOOL_CALL 输出约定。
     */
    private fun systemPromptWithTools(): String {
        val toolBlock = ToolRegistry.describeForPrompt()
        if (toolBlock.isBlank()) return SYSTEM_PROMPT
    ...
    fun decide(
        ...
        messages.put(
            JSONObject()
                .put("role", "system")
                .put("content", systemPromptWithTools())
        )
...
- **工具调用步骤必须用 tool_call 完成**：如果当前计划步骤的文字含"调用 X 工具"或"X 工具"字样（X = web_search / web_read / apk_install / file_ops），你**必须**输出 `{"action":"tool_call","tool_name":"X","args":{...},"plan_step":当前步}`，禁止改用 tap / open_app / search / input 等屏幕操作去"绕路"完成该步骤——屏幕点击永远无法完成工具调用步骤，只会让你跑偏到浏览器里瞎点。系统返回工具结果后，把 plan_step 推进到下一步。
...
{"thought":"用 web_search 工具搜今天的天气","action":"tool_call","tool_name":"web_search","args":{"query":"今天的天气"},"plan_step":1}
...
11. tool_call：调用系统注册的 Python 工具，字段 tool_name 填工具名，args 对象填参数（详见下方"可用工具"清单）。当任务文本里出现工具名（如 web_search、web_read、apk_install、file_ops），或任务本质是"搜一下/读一下/查一下/装一下"等可由工具直接完成的信息获取型任务时，**必须**用 tool_call 调用对应工具，禁止改用浏览器导航去手动访问该网站——浏览器路径慢且容易跑偏
12. finish：任务已经完成，或遇到无法自动处理的情况，结束运行
...
3. 第一步要根据任务性质决定（非常重要）：
   - **工具调用型任务**：如果任务文本里出现工具名（web_search、web_read、apk_install、file_ops），或任务本质是"搜一下/读一下/查一下/装一下"等信息获取型任务且不需要在手机 App 内操作，第一步直接写"调用 X 工具完成 Y（系统返回工具输出结果）"。例如任务"用 web_search 搜今天的天气"→第一步"调用 web_search 工具搜索今天的天气（系统返回搜索结果摘要）"。**禁止**把这种任务规划成"打开浏览器→输入网址→搜索"——那样既慢又容易跑偏，工具调用一步就能完成。
   - **App 操作型任务**：其余任务第一步固定写"打开目标 App"，并写明打开后应看到的首页标志。要根据任务选择手机上真实存在、最合适的 App：点外卖用美团或饿了么，买东西用淘宝/京东/拼多多，短视频/种草用抖音/小红书，搜信息用浏览器或对应内容 App，查火车票用铁路12306。如果用户明确说了 App 就用用户说的。
...
9. goal 用一句话概括任务最终要达成的结果（含用户的关键约束）。工具调用型任务的 goal 也要明确写出"通过 X 工具获取 Y"，不要写成"打开浏览器搜 Y"。
...
任务"用 web_search 工具搜一下今天的天气，告诉我结果"
{
  "target_app": "",
  "goal": "通过 web_search 工具获取今天的天气信息并报告给用户",
  "steps": [
    "调用 web_search 工具搜索今天的天气（系统返回搜索结果摘要）",
    "如需更详细信息，调用 web_read 工具读取其中一条结果的网页（系统返回网页正文）",
    "把今天天气的关键信息（气温/天气状况/降雨等）总结后输出 finish 结束任务（用户能看到总结）"
  ]
}
        """.trimIndent()
    }
